...
from src.vector_db.fill_deals_table import fill_deals_table
...
def mock_components(
    ...
    return {
        'openai_embedding_api': mocker.patch('src.vector_db.fill_deals_table.openai_embedding_api'),
        ...
        'connect': mocker.patch('src.vector_db.fill_deals_table.psycopg2.connect'),
...
def test_fill_deals_table_returns_true_if_pass_auth(
    ...
    # Act
    result = fill_deals_table(data, key, as_csv_file(aws_info), as_csv_file(database_info))
...
def test_fill_deals_table_exits_when_wrong_passkey_given(
        mock_components
) -> None:
    # Arrange
    data = [
    ...
    # Act
    result = fill_deals_table(data, key)
...
def test_fill_deals_table_exits_when_no_entered(
        mock_components
) -> None:
    # Arrange
    data = [
