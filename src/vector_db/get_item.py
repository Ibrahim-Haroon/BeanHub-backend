...
def get_item(
        ...
        embedding_cache: redis.Redis = None,
        database_csv_file: StringIO = None) -> str and bool:
    """
    ...
    def get_embedding() -> None:
        ...
        if embedding_cache and embedding_cache.exists(order):
            ...
            vector_embedding = openai_embedding_api(
                order, api_key if api_key else None)
            if embedding_cache:
        ...
        logging.info(
            "openai_embedding time %s:", {
                time.time() - openai_embedding_time})
    ...
    logging.info("start new db session time %s:",
                 {time.time() - set_session_time})

    def pg_register_vector() -> None:
        register_vector_time = time.time()
        register_vector(db_connection)
        logging.info(
            "register_vector time %s:", {
                time.time() - register_vector_time})
    ...
    if connection_pool:
        ...
        db_connection.close()
    logging.info("close db connection time %s:", {
                 time.time() - close_connection_time})
